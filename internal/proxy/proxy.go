...
func (proxy *Proxy) httpsHandler(response http.ResponseWriter, request *http.Request) {
	...
	for {
		...
		// Look for JSON request body
		// for _, val := range tunnelReq.Header.Values("Content-Type") {
		// 	if strings.EqualFold(val, "application/json") {
		// 		// Save JSON request body
		// 		body, bodyCopy, readErr := internalHttp.ReadBody(tunnelReq.Body)
		// 		tunnelReq.Body = bodyCopy
		// 		if readErr != nil {
		// 			log.WithError(readErr).Error("unable to read body of HTTPS request")
		// 			break
		// 		}
		// 		var bodyJson json.RawMessage = body
		// 		reqResp.Request.BodyJson = bodyJson
		...
		// TODO: There's definitely a cleaner way to do this. Make the same change in the httpHandler function as well.
		// RespBodyLoop:
		// 	for _, val := range tunnelResp.Header.Values("Content-Type") {
		// 		switch {
		// 		case strings.EqualFold(val, "application/json"):
		// 			// Save JSON request body
		// 			var bodyJson json.RawMessage
		// 			body, bodyCopy, readErr := internalHttp.ReadBody(tunnelResp.Body)
		// 			tunnelResp.Body = bodyCopy
		// 			if readErr != nil {
		// 				log.WithError(readErr).Error("unable to read body of HTTPS response")
		//
		// 				// Break from for loop
		// 				break RespBodyLoop
		// 			}
		...
		//
		// 			// Decode and save body contents
		// 			switch strings.ToLower(tunnelResp.Header.Get("content-encoding")) {
		// 			case "gzip":
		// 				// Decode and save the data
		// 				var decodeErr error
		// 				bodyJson, decodeErr = internalHttp.DecodeGzip(body)
		// 				if decodeErr != nil {
		// 					log.WithError(decodeErr).Error("unable to decode gzip-encoded body of HTTPS response")
		//
		// 					// Break from for loop
		// 					break RespBodyLoop
		// 				}
		...
		// 				bodyJson, decodeErr = internalHttp.DecodeBrotli(body)
		// 				if decodeErr != nil {
		// 					log.WithError(decodeErr).Error("unable to decode brotli-encoded body of HTTPS response")
		//
		// 					// Break from for loop
		// 					break RespBodyLoop
		// 				}
		...
		// 				bodyJson, decodeErr = internalHttp.DecodeDeflate(body)
		// 				if decodeErr != nil {
		// 					log.WithError(decodeErr).Error("unable to decode deflate-encoded body of HTTPS response")
		//
		// 					// Break from for loop
		// 					break RespBodyLoop
		// 				}
		// 				reqResp.Response.BodyJson = bodyJson
		// 			default:
		// 				if encodingType := tunnelResp.Header.Get("content-encoding"); len(encodingType) > 0 {
		// 					// Unsupported content-encoding type
		// 					log.Errorf("content-encoding type not supported in HTTPS response: %s", encodingType)
		//
		// 					// Break from for loop
		// 					break RespBodyLoop
		// 				}
